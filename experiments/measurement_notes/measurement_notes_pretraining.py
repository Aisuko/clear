...
def main( args, train_dataloader, val_dataloader, test_dataloader, val_zeroshot_dataloader, test_zeroshot_dataloader):
    ...
    for epoch in range(args.epochs):
        ...
        for idx, notes_dict in enumerate(train_dataloader):
            ...
            measurement_input["x"] = notes_dict["measurement_data"].cuda(gpu, non_blocking=True)

            notes_input = {"x": notes_dict["note_texts_tokenized"].cuda(gpu, non_blocking=True)}

            model_input = [measurement_input, notes_input]

            with torch.cuda.amp.autocast(), torch.backends.cuda.sdp_kernel(enable_flash=True):
        ...
        if (epoch + 1) % 5 == 0:
            ...
            if args.rank == 0:
                ...
                torch.save(model.state_dict(), os.path.join(exp_path, f"model_{epoch + 1}.pth"))
    ...
    score_test_m2t, score_test_t2m, eval_loss = evaluation( model_without_ddp, test_dataloader,gpu,args)
    ...
    if args.rank == 0:
        ...
        (zeroshot_auc_roc,zeroshot_auc_pr,zeroshot_bal_acc,) = zeroshot_eval(model_without_ddp,test_zeroshot_dataloader,train_dataloader.dataset.tokenizer)
...
@torch.no_grad()
def zeroshot_eval( model, zeroshot_dataloader,tokenizer):
    model.eval()
...
def evaluation(model, data_loader, device, args):
    ...
    for idx, notes_dict in enumerate(data_loader):
        ...
        with torch.backends.cuda.sdp_kernel(enable_flash=True):
            ...
            # torch.einsum is used to compute the cosine similarity
    ...
    # Process the entire matrix (no distributed partitioning)
    for i, sims in enumerate(sims_matrix):
        topk_sim, topk_idx = sims.topk(k=args.k_test, dim=0)
        score_matrix_m2t[i, topk_idx] = topk_sim
    ...
    for i, sims in enumerate(sims_matrix):
        topk_sim, topk_idx = sims.topk(k=args.k_test, dim=0)
        score_matrix_t2m[i, topk_idx] = topk_sim
...
if __name__ == "__main__":
    ...
    train_measurement_transform = transforms.Compose([ShuffleTransform(args.measurement_max_seq_len)])
    test_measurement_transform = transforms.Compose([ShuffleTransform(args.measurement_max_seq_len)])

    # mimic3_benchmark_root: /workspaces/multimodal-mimic/mimic3-benchmarks
    # mimic3_root: /workspaces/multimodal-mimic/raw-mimic3
    ...
    data_exist=os.path.exists("train_notes_dataset.pkl") and os.path.exists("val_notes_dataset.pkl") and os.path.exists("test_notes_dataset.pkl")
    ...
    if not data_exist:
        ...
        train_notes_dataset.save_to_disk("train_notes_dataset.pkl")
        val_notes_dataset.save_to_disk("val_notes_dataset.pkl")
        test_notes_dataset.save_to_disk("test_notes_dataset.pkl")
    else:
        train_notes_dataset = MIMICIIINoteDataset.load_from_disk("train_notes_dataset.pkl")
        val_notes_dataset = MIMICIIINoteDataset.load_from_disk("val_notes_dataset.pkl")
        test_notes_dataset = MIMICIIINoteDataset.load_from_disk("test_notes_dataset.pkl")
    ...
    val_zeroshot_dataset = IHMDataset(root, customListFile=os.path.join(root, val_listfile), train=True)

    val_zeroshot_dataset = IHMDataset(root, customListFile=os.path.join(root, test_listfile), train=False)
    ...
    train_dataloader = DataLoader(
        ...
        pin_memory=True,
        drop_last=True,
        sampler=RandomSampler(train_notes_dataset),
    )
    ...
    val_dataloader = DataLoader(
        ...
        pin_memory=True,
        sampler=RandomSampler(val_notes_dataset),
    )
    ...
    test_dataloader = DataLoader(
        ...
        pin_memory=True,
        sampler=RandomSampler(test_notes_dataset),
    )
    ...
    main(args, train_dataloader, val_dataloader, test_dataloader, val_zeroshot_dataloader, test_zeroshot_dataloader)
